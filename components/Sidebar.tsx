...
const Sidebar: React.FC<SidebarProps> = ({ activeView, onViewChange, user }) => {
  ...
  return (
    <>
      ...
      {/* Mobile Bottom Navigation - DB fix included */}
      <nav className="md:hidden fixed bottom-0 left-0 w-full bg-black/95 backdrop-blur-2xl border-t border-emerald-500/20 px-1 py-3 flex justify-around items-center z-[100] safe-area-bottom">
        {menuItems.map((item) => (
          <button
            key={item.id}
            onClick={() => onViewChange(item.id)}
            className={`flex flex-col items-center gap-1.5 min-w-[60px] transition-all duration-300 ${
              activeView === item.id ? 'text-emerald-400 scale-110' : 'text-slate-600'
            }`}
          >
            <div className={`p-1.5 rounded-lg transition-colors ${activeView === item.id ? 'bg-emerald-500/10' : ''}`}>
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={item.icon} />
              </svg>
            </div>
            <span className="text-[7px] font-black mono uppercase tracking-tighter">{item.label}</span>
          </button>
        ))}
      </nav>
    </>
